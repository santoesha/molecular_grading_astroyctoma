...
cnv.metadata <- readRDS("data/clean/glass/rds/heidelberg_binvals.Rds") %>% 
...
cnv <- readRDS("data/clean/glass/rds/heidelberg_binvals.Rds") %>% 
       dplyr::filter(! cnv.segments.chrom %in% c("chrX", "chrY")) %>% 
       dplyr::mutate(cnv.segments.id = paste0('cnv.segment.',cnv.segments.id,':',cnv.segments.start,'-',cnv.segments.end)) %>%
       ...
       dplyr::filter(BatchID %in% meta_glass$BatchID)

oncovals <- readRDS("data/clean/glass/rds/heidelberg_genevals.Rds") 
...
identical(rownames(data),meta_glass$BatchID)

cnv.cor.lgc <- data.frame(cor.t.cnv.lgc = apply(data,2, function(vec) {return( cor.test(meta_glass$log_AIDH_cal_ratio, 
...
c <- plt.disc.cnv.out %>%
    dplyr::mutate(label = case_when(chr == "chr4" & start > 55.09 & end < 55.16 & wilc.padj < 0.05  & 
                                    ...
                                    chr == "chr4" & start > 57.49 & end < 57.56 & wilc.padj < 0.05 &
                                      test == "AMP" ~ "HOPX",
                                    chr == "chr12" & start > 58.15 & end < 58.21 & wilc.padj < 0.05 &
                                      test == "AMP" ~ "CDK4",
                                    chr == "chr9" & start > 21.80 & end < 21.96 & wilc.padj < 0.05 &
                                      test == "HD" ~ "CDKN2AB",
                                    chr == "chr9" & start > 2.00 & end < 2.06 & wilc.padj < 0.05 &
                                      test == "HD" ~ "SMARCA2",
                                    chr == "chr2" & start > 240.24 & end < 240.31 & wilc.padj < 0.05 &
                                      test == "HD" ~ "HDAC4",
                                    chr == "chr13" & start > 48.85 & end < 48.91 & wilc.padj < 0.05 &
                                      test == "HD" ~ "RB1",
...
# saveRDS(c, 'output/tables/rds/cnv_glass_tab.Rds')
...
ggplot(c, aes(x=pos, y=log_padj, label = label, col = p.sign)) +
  ...
  labs(x = "Chromosomal position", 
       ...
       title = paste0("GLASS-NL (n=", nrow(cnv), ")")) +
  geom_point() +
  theme_bw() +
...
ggsave("output/figures/paper/sf4/glass_cnv_lgc.pdf", width = 17, height = 5)
...
identical(rownames(data_num),meta_glass$BatchID)

cor.onco.lgc <- data.frame(cor.t.onco.lgc = apply(data_num,2, function(vec) {return( cor.test(meta_glass$log_AIDH_cal_ratio, 
                                                                                              as.numeric(vec), method = "spearman")$estimate) })) %>% 
  dplyr::mutate(cnv.segments.id = rownames(.)) %>% 
  dplyr::left_join(.,cnv.metadata )
...
surv_meta <- meta_glass %>% 
  dplyr::filter(Resectie != 1) %>% 
  dplyr::group_by(GLASS_ID) %>% 
  dplyr::filter(Resectie == min(Resectie)) %>% 
