...
gtf <- read.delim('~/mnt/neuro-genomic-ro/belob/ref/hg19/gencode.v29lift37.annotation.gtf.gz',comment.char = "#",sep="\t",header=F) %>% 
...
c_sig_tcga <- readRDS("output/clean/tcga/rds/significant_cnv_lgc_tcga.Rds")
...
cnv.metadata <- readRDS("data/clean/catnon/rds/heidelberg_binvals.Rds") %>% 
...
cnv <- readRDS("data/clean/catnon/rds/heidelberg_binvals.Rds") %>% 
       dplyr::filter(! cnv.segments.chrom %in% c("chrX", "chrY")) %>% 
       dplyr::mutate(cnv.segments.id = paste0('cnv.segment.',cnv.segments.id,':',cnv.segments.start,'-',cnv.segments.end)) %>%
...
oncovals <- readRDS("data/clean/catnon/rds/heidelberg_genevals.Rds") 
...
identical(rownames(data),meta_catnon$BatchID)

cnv.cor.lgc <- data.frame(cor.t.cnv.lgc = apply(data,2, function(vec) {return( cor.test(meta_catnon$log_AIDH_cal_ratio, 
...
c <- plt.disc.cnv.out %>%
     dplyr::mutate(label = case_when(chr == "chr4" & start > 55.09 & end < 55.16 & wilc.padj < 0.05  & 
                                  ...
                                  chr == "chr12" & start > 58.09 & end < 58.16 & wilc.padj < 0.05 &
                                    test == "AMP" ~ "CDK4",
                                  chr == "chr9" & start > 21.94 & end < 22.01 & wilc.padj < 0.05 &
                                    test == "HD" ~ "CDKN2AB",
                                  chr == "chr9" & start > 2.05 & end < 2.21 & wilc.padj < 0.05 &
                                    test == "HD" ~ "SMARCA2",
                                  chr == "chr2" & start > 240.24 & end < 240.31 & wilc.padj < 0.05 &
                                    test == "HD" ~ "HDAC4",
                                  chr == "chr13" & start > 48.90 & end < 49.13 & wilc.padj < 0.05 &
                                    test == "HD" ~ "RB1",
...
# saveRDS(c, 'output/tables/rds/cnv_catnon_tab.Rds')
...
ggplot(c, aes(x=pos, y=log_padj, label = label, col = p.sign)) +
      ...
      labs(x = "Chromosomal position", 
           ...
           title = paste0("CATNON (n=", nrow(cnv), ")")) +
      geom_point() +
      theme_bw() +
...
# ggsave("output/figures/paper/sf4/catnon_cnv_lgc.pdf", width = 17, height = 5)


subsetByOverlaps(hg19IdeogramCyto, genesymbol["SMAD3",])

# Oncovals Heidelberg -----

# ** numeric ----
data_num <- meta_catnon %>% 
            dplyr::select(BatchID) %>% 
            dplyr::left_join(., oncovals, by = c("BatchID" = "BatchID")) %>% 
            tibble::column_to_rownames('BatchID')  

identical(rownames(data_num),meta_catnon$BatchID)

cor.onco.lgc <- data.frame(cor.t.onco.lgc = apply(data_num,2, function(vec) {return( cor.test(meta_catnon$log_AIDH_cal_ratio, 
                                                                                              as.numeric(vec), method = "spearman")$estimate) })) %>% 
  dplyr::mutate(cnv.segments.id = rownames(.)) %>% 
  dplyr::left_join(.,cnv.metadata )
...
disc.out <- data_bin %>% 
            ...
            dplyr::left_join(., meta_catnon) %>% 
            dplyr::select(log_AIDH_cal_ratio, alteration, status, BatchID) %>% 
            dplyr::mutate(group = ifelse(status == "NONE", 1, 2)) %>% 
            dplyr::group_by(alteration) %>%  
            dplyr::mutate(n_levels = n_distinct(group)) %>% 
            dplyr::filter(n_levels > 1) %>% 
            dplyr::do(w = wilcox.test(log_AIDH_cal_ratio ~ group, data= .)) %>% 
...
surv_meta <- meta_catnon %>% 
             dplyr::left_join(., data_del) %>% 
             dplyr::mutate(`cnv.segment.chr9-0120:21950001-22000000` = factor(`cnv.segment.chr9-0120:21950001-22000000`, 
                                                                              levels = c("NO-DEL", "DEL"))) %>% 
             dplyr::mutate(`cnv.segment.chr13-0314:49050001-49100000` = factor(`cnv.segment.chr13-0314:49050001-49100000`, 
                                                                             levels = c("NO-DEL", "DEL"))) %>% 
             dplyr::mutate(`cnv.segment.chr9-0016:2050001-2200000` = factor(`cnv.segment.chr9-0016:2050001-2200000`, 
                                                                             levels = c("NO-DEL", "DEL")))

fit_cdkn2ab <- coxph(Surv(time = Survival, event = as.numeric(Deceased)) ~ `cnv.segment.chr9-0120:21950001-22000000`, data = surv_meta)
fit_rb1 <- coxph(Surv(time = Survival, event = as.numeric(Deceased)) ~ `cnv.segment.chr13-0314:49050001-49100000`, data = surv_meta)
fit_smarca2 <- coxph(Surv(time = Survival, event = as.numeric(Deceased)) ~ `cnv.segment.chr9-0016:2050001-2200000`, data = surv_meta)
...
surv_meta <- meta_catnon %>% 
             dplyr::left_join(., data_amp) %>% 
              dplyr::mutate(`cnv.segment.chr12-0598:58100001-58150000` = factor(`cnv.segment.chr12-0598:58100001-58150000`, 
                                                                             levels = c("NO-AMP", "AMP"))) %>% 
              dplyr::mutate(`cnv.segment.chr4-0410:55100001-55150000` = factor(`cnv.segment.chr4-0410:55100001-55150000`, 
                                                                             levels = c("NO-AMP", "AMP")))

fit_cdk4 <- coxph(Surv(time = Survival, event = as.numeric(Deceased)) ~ `cnv.segment.chr12-0598:58100001-58150000`, data = surv_meta)
fit_pdgfra <- coxph(Surv(time = Survival, event = as.numeric(Deceased)) ~ `cnv.segment.chr4-0410:55100001-55150000`, data = surv_meta)
